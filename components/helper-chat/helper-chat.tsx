...
export function HelperChat() {
  ...
  React.useEffect(() => {
    ...
    else if (prevPath.current !== null && pathname !== prevPath.current && open) {
      setOpen(false)
            ...
            <header className="relative flex items-center justify-between px-4 py-3 bg-gradient-to-r from-emerald-600 to-emerald-700 text-white">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button variant="ghost" size="icon" aria-label="Close" onClick={() => closePanel()}>
                  <Image src={CloseIcon} alt="" width={16} height={16} className="h-4 w-4" aria-hidden={true} />
                </Button>
              </div>
            </header>
...
function Suggestions({ onPick }: { onPick: (text: string) => void }) {
  ...
  else if (pathname?.startsWith("/dashboard/media-universe")) items = [...mediaUniverse, ...base]
  else if (pathname?.startsWith("/dashboard/channel-flow")) items = [...channelFlow, ...base]
  else if (pathname?.startsWith("/dashboard/category-signals")) items = [...categorySignals, ...base]
  else if (pathname?.startsWith("/dashboard/client-pulse")) items = [...clientPulse, ...base]
