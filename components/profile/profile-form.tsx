...
const ProfileSchema = z.object({
  ...
  return next === confirm
}, { message: "Passwords must match and be at least 8 characters", path: ["confirmNewPassword"] })
...
export function ProfileForm() {
  ...
  const inputClasses = "h-11 rounded-none border border-black/20 bg-[#cfd8d5] px-3 text-sm text-black placeholder:text-black/60 focus-visible:border-[#1b74ff] focus-visible:ring-4 focus-visible:ring-[#1b74ff]/40 focus-visible:ring-offset-0 dark:h-12 dark:border-[#1d4c39] dark:bg-[#04211a] dark:text-white dark:placeholder:text-white/60 dark:focus-visible:border-[#66ffa3] dark:focus-visible:ring-2 dark:focus-visible:ring-[#66ffa3]/40"
  const labelClasses = "text-sm font-semibold text-black dark:text-white"
  const buttonBase = "h-11 rounded-none px-7 text-sm font-semibold transition-colors"
  ...
  return (
    <div className="w-full max-w-4xl space-y-10 px-6 py-10 text-black md:px-10 dark:rounded-2xl dark:border dark:border-white/10 dark:bg-[#031611]/90 dark:text-white dark:shadow-[0_25px_70px_-45px_rgba(102,255,163,0.6)]">
      <div className="space-y-2">
        <h1 className="space-grotesk text-[2.25rem] font-bold leading-tight text-black md:text-[2.5rem] dark:text-[#66ffa3]">Account Settings</h1>
        <p className="text-sm text-black/70 dark:text-white/70">Update your profile, preferences, and security settings here.</p>
      </div>
      ...
      <div className="flex flex-col items-start gap-5 md:flex-row md:items-center md:gap-8">
        ...
        <div className="flex flex-col gap-3.5">
          ...
          <p className="text-xs text-black/60 dark:text-white/60">We support PNGs, JPEGs and GIFs under 2MB.</p>
        </div>
      </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-10">
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
            <FormField name="firstName" control={form.control} render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="John" className={inputClasses} disabled={loading || isSubmitting} {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField name="lastName" control={form.control} render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Doe" className={inputClasses} disabled={loading || isSubmitting} {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )} />
          ...
          <FormField name="email" control={form.control} render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="email" placeholder="doe@gmail.com" className={inputClasses} disabled={loading || isSubmitting} {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )} />
          ...
          <div className="space-y-5">
            <h2 className="space-grotesk text-xl font-semibold text-black dark:text-[#66ffa3]">Password</h2>
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
              <FormField name="newPassword" control={form.control} render={({ field }) => (
                <FormItem>
                  <FormLabel className={labelClasses}>New Password</FormLabel>
                  <FormControl>
                    <Input type="password" placeholder="••••••••" className={inputClasses} disabled={loading || isSubmitting} {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
